...
case "tambah";
	...
	$sql1 = "insert into upa (nip, nama, tempat_lahir, tanggal_lahir, alamat, nomor_telepon, jenis_kelamin, agama, photo)
	...
	if ($hasil1) {
		if ($error > 0) {
			...
			echo "<script>alert('Data UPA Sudah ditambahkan');
				document.location.href = '../../index2.php?modul=upa&aksi=tampil'; </script>";
...
case "edit":
	...
	if(empty($data_photo)){
		$sql = "update upa set nip = '$nip', nama = '$nama', tempat_lahir = '$tempat_lahir', tanggal_lahir = '$tanggal_lahir',
	...
	if ($hasil) {
		if ($error > 0) {
			...
			echo "<script>alert('Data UPA Sudah diedit');
				document.location.href = '../../index2.php?modul=upa&aksi=tampil'; </script>";
		} else {
...
case "hapus":
	$sql1 = "delete from upa where nip = '$_GET[nip]'";
	$hasil1 = mysql_query($sql1);
	...
	if ($hasil1) {
		if ($error > 0) {
			...
			echo "<script>alert('Data UPA Berhasil dihapus!');
				document.location.href = '../../index2.php?modul=upa&aksi=tampil'; </script>";
				unlink("../../$_GET[photo]");
		} else {
