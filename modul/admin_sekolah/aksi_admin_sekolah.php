...
case "tambah";
	...
	$sql1 = "insert into admin_sekolah (nip, nama, tempat_lahir, tanggal_lahir, alamat, nomor_telepon, jenis_kelamin, agama, photo, id_sekolah)
			values ('$nip', '$nama', '$tempat_lahir', '$tanggal_lahir', '$alamat', '$nomor_telepon', '$jenis_kelamin', '$agama', '$new_name', '$id_sekolah')";
	...
	if ($hasil1) {
		if ($error > 0) {
			...
			echo "<script>alert('Data Mater Admin Sekolah Sudah ditambahkan');
				document.location.href = '../../index2.php?modul=admin_sekolah&aksi=tampil'; </script>";
...
case "edit":
	...
	if(empty($data_photo)){
		$sql = "update admin_sekolah set nip = '$nip', nama = '$nama', tempat_lahir = '$tempat_lahir', tanggal_lahir = '$tanggal_lahir',
				alamat = '$alamat', nomor_telepon = '$nomor_telepon', jenis_kelamin = '$jenis_kelamin', agama = '$agama', 
				photo = '$nama_foto_baru', id_sekolah = '$id_sekolah' where nip = '$nip_awal'";
		...
		$sql = "update admin_sekolah set nip = '$nip', nama = '$nama', tempat_lahir = '$tempat_lahir', tanggal_lahir = '$tanggal_lahir',
				alamat = '$alamat', nomor_telepon = '$nomor_telepon', jenis_kelamin = '$jenis_kelamin', agama = '$agama', 
				photo = '$new_name', id_sekolah = '$id_sekolah' where nip = '$nip_awal'";
		$hasil = mysql_query($sql);
		move_uploaded_file($photo, $name);
		if ($nip==$nip_awal) {
	...
	if ($hasil) {
		if ($error > 0) {
			...
			echo "<script>alert('Data Admin Sekolah Sudah diedit');
				document.location.href = '../../index2.php?modul=admin_sekolah&aksi=tampil'; </script>";
		} else {
...
case "hapus":
	$sql1 = "delete from admin_sekolah where nip = '$_GET[nip]'";
	$hasil1 = mysql_query($sql1);
	...
	if ($hasil1) {
		if ($error > 0) {
			...
			echo "<script>alert('Data Admin Sekolah Berhasil dihapus!');
				document.location.href = '../../index2.php?modul=admin_sekolah&aksi=tampil'; </script>";
				unlink("../../$_GET[photo]");
		} else {
